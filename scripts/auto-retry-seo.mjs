...
# SEO改善ルール(重要・2026-08-07改訂)
- (最重要)Shorts再生の9割以上はおすすめフィード経由で、検索経由は1割未満というデータがある。titleを検索キーワードで埋めるより、視聴維持率(最後まで見たくなるか)を優先すること
- titleは核心・具体的な結論(数字や答え)を言い切らないこと。「〜は3つ！」のような断定・ネタバレは視聴完了率を下げるため避け、「まさかの理由」のように核心は動画を見ないとわからない状態を保つ。他の同ジャンル動画と矛盾しない範囲にする。30字以内
- 感嘆符・疑問符は必ず全角(！／？)を使うこと。半角(!／?)は使わない。二重引用符(")は使わない
- tagsは、最も重要・具体的なキーワードを先頭から順に8個程度。検索ボリュームが見込める一般語(雑学,豆知識等)と、固有名詞・専門用語を両方含める
- descriptionHookは、検索結果・スマホ画面で最初に表示される1文。内容が一目で伝わるようにする(ただしtitle同様、核心の答えまでは書かない)
- seoNotesには、今回どう視聴維持率・エンゲージメントを意識してSEO改善したかを日本語1〜2文で具体的に説明すること
...
# 出力JSON形式
{
  ...
  if (rowIndex !== -1) {
    ...
    const retryNote = `[${formatJst(now)}] SEO自動再試行(${entry.seoRetryCount}回目): 1日あたり${viewsPerDay.toFixed(
      1
