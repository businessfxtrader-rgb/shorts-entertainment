...
- 検索キーワード最適化(tags・descriptionHook)は引き続き重視すること
- ただしtitleは核心・具体的な結論(数字や答え)を言い切らないこと。「〜は3つ！」のような断定・ネタバレは視聴完了率を下げてスキップされやすくなるため避け、「まさかの理由」のように核心は動画を見ないとわからない状態を保つ。検索キーワードは主にtags・descriptionHook側で担う。他の同ジャンル動画と矛盾しない範囲にする。30字以内
